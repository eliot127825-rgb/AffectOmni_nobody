...
class RepeatRandomSampler(Sampler):
    ...
    def __iter__(self):
        # Generate all repeated indexes at once using list comprehension
        # This ensures proper distribution in multi-GPU training
        indexes = [
            idx
...
class VLMGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        # Debug: check conversation format
        if self.state.global_step == 0 and self.accelerator.is_main_process:
            ...
            if isinstance(inputs[0]['prompt'], list):
                ...
                if len(inputs[0]['prompt']) > 1:
                    print(f"Second message keys: {inputs[0]['prompt'][1].keys() if isinstance(inputs[0]['prompt'][1], dict) else 'not a dict'}")
            print(f"================================\n")
        
        prompts_text = self.vlm_module.prepare_prompt(self.processing_class, inputs)
        use_audio_in_video = getattr(self, 'use_audio_in_video', False)
        # print(prompts_text)
        ...
        # Debug: print multimodal data status
        if self.state.global_step == 0 and self.accelerator.is_main_process:
            print(f"\n=== GRPO Trainer Debug ===")
            ...
            if self.state.global_step == 0 and self.accelerator.is_main_process:
                ...
                if hasattr(unwrapped_model, 'thinker'):
                    ...
                    if hasattr(unwrapped_model.thinker, 'model'):
                        print(f"  unwrapped_model.thinker.model._gradient_checkpointing_func: {getattr(unwrapped_model.thinker.model, '_gradient_checkpointing_func', 'N/A')}")
                print(f"==========================================\n")
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                # Comparative scoring: gather all completions in multi-GPU training,
                # score them on the main process, then scatter results back to each GPU
                ...
                # Gather data from all GPUs to the main process
                all_prompts = gather_object(prompts)
                all_completions = gather_object(completions)
                all_reward_kwargs = {}
                for key in reward_kwargs:
                    all_reward_kwargs[key] = gather_object(reward_kwargs[key])
                
                # Call reward function only on the main process (can see all answers for comparison)
                if self.accelerator.is_main_process:
                    ...
                    output_reward_func = None
                
                # Broadcast results to all GPUs
                output_reward_func = broadcast_object_list([output_reward_func])[0]
                
                # Compute the data range for the current GPU
                world_size = self.accelerator.num_processes
                local_rank = self.accelerator.process_index
                total_samples = len(output_reward_func)
                ...
                # Extract rewards for the current GPU
                local_rewards = output_reward_func[start_idx:end_idx]
                rewards_per_func[:, i] = torch.tensor(local_rewards, dtype=torch.float32, device=device)
    ...
    def _get_train_sampler(self, train_dataset=None) -> Sampler:
        ...
        # For GRPO, the sampler should repeat each sample num_generations times
        # The batch_size for the sampler should match per_device_train_batch_size
        # since DataLoader will fetch per_device_train_batch_size samples at a time
